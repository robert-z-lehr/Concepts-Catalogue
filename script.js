...
function styleTabs(activeIndex) {
...
function getCarouselImages(concept) {
  return Array.isArray(concept.images) && concept.images.length > 0 ? concept.images : [DEFAULT_CONCEPT_IMAGE];
}
...
function renderConcept(index) {
  ...
  document.body.dataset.art = concept.art;
  ...
  contentContainer.innerHTML = `
    <section class="hero">
      <figure class="concept-image-frame carousel" aria-label="Image carousel for ${concept.title}" title="Click image area to expand or shrink">
        ...
        <figcaption class="carousel-count">1 / ${currentCarouselImages.length}</figcaption>
      </figure>
      <div class="concept-meta"><p class="acronym">${concept.acronym}</p><h2>${concept.title}</h2><p class="tagline">${concept.description}</p></div>
    </section>
    <div class="framework-grid">${cards}</div>
    <section class="feedback"><h2>Feedback</h2><form class="feedback-form"><input type="hidden" name="concept" value="${concept.title}" /><label>Optional Email<input type="email" name="email" placeholder="your@email.com" /></label><label>Feedback<textarea name="message" required></textarea></label><button type="submit">Submit Feedback</button><p class="form-status"></p></form></section>`;
...
function attachFeedbackHandler() {
  ...
  form.addEventListener('submit', async event => {
    ...
    formData.append('submitted_at_local', new Date().toLocaleString('en-US', { timeZone: 'America/Chicago', dateStyle: 'full', timeStyle: 'long' }));
